...
#define JSITEM_PRS_FT__PARAM_CHECK(func)                                       \
    ...
    {                                                                          \
        ...
        CHECK_EQ(func(&item_json, 0U, NULL, &buf_len), SWICC_RET_PARAM_BAD);   \
        CHECK_EQ(func(&item_json, 0U, &buf, NULL), SWICC_RET_PARAM_BAD);       \
    } while (0)
...
{
    cJSON item_json = {0U};
    swicc_fs_file_raw_st file = {0U};
    CHECK_EQ(jsitem_prs_file_raw(NULL, &file, 0U), SWICC_RET_PARAM_BAD);
    CHECK_EQ(jsitem_prs_file_raw(&item_json, NULL, 0U), SWICC_RET_PARAM_BAD);
}
...
TEST(fs_diskjs, jsitem_prs_file_raw__file_empty)
{
    ...
    {
        swicc_fs_file_raw_st file_act = {0};
        /* Parsing empty JSON object should succeed as no field is mandatory. */
        swicc_ret_et const ret_empty = jsitem_prs_file_raw(file, &file_act, 0U);
        CHECK_EQ(ret_empty, SWICC_RET_SUCCESS);
        cJSON_Delete(file);
    }
}
...
TEST(fs_diskjs, jsitem_prs_file_raw__file_w_id)
{
    ...
    swicc_fs_id_kt const id = 0x0CDE;
    ...
    {
        swicc_fs_file_raw_st file_act = {0};
        swicc_fs_file_raw_st const file_exp = {
            .hdr_item = {0U},
            .hdr_item.offset_prel = offset_prel,
            .hdr_file =
                {
                    .id = id,
                    .sid = SWICC_FS_SID_MISSING,
                },
        };

        swicc_ret_et const ret =
            jsitem_prs_file_raw(file, &file_act, offset_prel);
        CHECK_EQ(ret, SWICC_RET_SUCCESS);
        CHECK_BUF_EQ(&file_act, &file_exp, sizeof(swicc_fs_file_raw_st));
        cJSON_Delete(file);
    }
}
...
TEST(fs_diskjs, jsitem_prs_file_raw__file_w_sid)
{
    ...
    swicc_fs_sid_kt const sid = 0xC9;
...
TEST(fs_diskjs, jsitem_prs_file_raw__file_w_id_sid)
{
    ...
    swicc_fs_id_kt const id = 0x3F12;
    swicc_fs_sid_kt const sid = 0x1F;
...
TEST(fs_diskjs, jsitem_prs_type_str__param_check)
{
    ...
    CHECK_EQ(jsitem_prs_type_str(NULL, 0U, &type), SWICC_RET_PARAM_BAD);
...
TEST(fs_diskjs, jsitem_prs_type_str__invalid)
{
         ...
         type_str_idx < sizeof(type_str) / sizeof(type_str[0U]); ++type_str_idx)
    {
        ...
        CHECK_EQ(jsitem_prs_type_str(type_str[type_str_idx],
                                     ...
                                     &type),
                 SWICC_RET_SUCCESS);
        CHECK_EQ(type, SWICC_FS_ITEM_TYPE_INVALID);
    }
}
...
TEST(fs_diskjs, jsitem_prs_type_str__valid)
{
    ...
    {
        swicc_fs_item_type_et type_act;
...
TEST(fs_diskjs, jsitem_prs_demux__item_empty)
{
    ...
    uint8_t buf;
    uint32_t buf_len = sizeof(buf);
    CHECK_EQ(jsitem_prs_demux(item_json, 0U, &buf, &buf_len), SWICC_RET_ERROR);
...
TEST(fs_diskjs, disk_json_prs__param_check)
{
    ...
    CHECK_EQ(disk_json_prs(NULL, &disk_json), SWICC_RET_PARAM_BAD);
    CHECK_EQ(disk_json_prs(&disk, NULL), SWICC_RET_PARAM_BAD);
}
...
TEST(fs_diskjs, disk_json_prs__data)
{
    ...
    TEST_DATA_FOREACH("test/data/disk/", {
        ...
        {
            swicc_ret_et const ret_disk_prs = disk_json_prs(&disk, obj);
            CHECK_EQ(ret_disk_prs, SWICC_RET_SUCCESS);
            (void)buf_out_len;
            (void)buf_out;
            ...
            else
            {
                ...
                CHECK_EQ(disk.lutid.size_item1, sizeof(swicc_fs_id_kt));
            ...
            if (ret_disk_prs == SWICC_RET_SUCCESS)
            {
