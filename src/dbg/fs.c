    ...
    *buf_str_len = 0;
    ...
    case UICC_FS_ITEM_TYPE_FILE_EF_CYCLIC: {
        ...
        if (uicc_fs_file_hdr_prs(
                (uicc_fs_file_hdr_raw_st *)&disk->buf[item->offset_start],
    ...
    case UICC_FS_ITEM_TYPE_FILE_DF: {
        ...
        if (item->offset_start + sizeof(uicc_fs_file_hdr_raw_st) > UINT32_MAX)
        {
            return ret;
        }
        ...
        uint32_t const data_offset_start =
            (uint32_t)(item->offset_start + sizeof(uicc_fs_file_hdr_raw_st));
        ...
        while (data_idx < data_len)
        {
            ...
            data_item.offset_start = data_offset_start + data_idx;
            ...
            ret_item = fs_item_str(&buf_str[buf_size - buf_unused_len],
                                   &buf_str_len_nested, disk, &data_item,
                                   (uint8_t)(depth + 1U));
            if (ret_item != UICC_RET_SUCCESS)
            {
                break;
    ...
    case UICC_FS_ITEM_TYPE_FILE_EF_TRANSPARENT: {
        ...
        for (uint32_t data_idx = 0U; data_idx < data_len; ++data_idx)
        {
            ...
            uint8_t const data_byte =
                disk->buf[item->offset_start + sizeof(uicc_fs_file_hdr_raw_st) +
                          data_idx];
                            ...
                            uicc_fs_disk_st const *const disk)
{
    ...
    *buf_str_len = 0U;
    ...
    uint16_t buf_unused_len = buf_size;
    while (disk_idx < disk->root.len)
    {
        uicc_fs_item_hdr_raw_st *const item_hdr =
            (uicc_fs_item_hdr_raw_st *)&disk->buf[disk_idx];
        ...
        item.offset_start = disk_idx;

        uint16_t dbg_str_len = buf_unused_len;
        uicc_ret_et ret_item = fs_item_str(&buf_str[buf_size - buf_unused_len],
                                           &dbg_str_len, disk, &item, 0U);
        if (ret_item != UICC_RET_SUCCESS)
        {
            return ret_item;
        }
