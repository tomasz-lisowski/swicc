                                  ...
                                  uicc_apdu_cmd_st const *const apdu_cmd)
{
    ...
    int bytes_written =
        snprintf(buf_str, *buf_str_len,
                 ...
                 "\n  (P2 0x%x))",
                 // clang-format on
                 uicc_dbg_apdu_cla_ccc_str(apdu_cmd->hdr.class),
                 uicc_dbg_apdu_cla_sm_str(apdu_cmd->hdr.class),
                 uicc_dbg_apdu_cla_type_str(apdu_cmd->hdr.class),
                 apdu_cmd->hdr.class.lchan,
                 apdu_cmd->hdr.class.type == UICC_APDU_CLA_TYPE_INTERINDUSTRY
                     ? uicc_dbg_apdu_ins_str(apdu_cmd->hdr.instruction)
                     : "???",
